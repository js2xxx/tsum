...
impl SumDebug for () {
    unsafe fn debug(this: &Self::Repr, _: u8) -> &dyn fmt::Debug {
        &this.0
    }
}
...
pub(super) trait SumDisplay: SumList {
    unsafe fn display(this: &Self::Repr, tag: u8) -> &dyn fmt::Display;
}

impl SumDisplay for () {
    unsafe fn display(this: &Self::Repr, _: u8) -> &dyn fmt::Display {
        &this.0
    }
}
